...
class Envira_Gallery_Updater {
    ...
    public function __construct( array $config ) {
        ...
        $timestamp = get_option( 'envira_gallery_updates_' . $this->plugin_slug );
        if ( ! $timestamp ) {
            $timestamp = strtotime( '+12 hours' );
            update_option( 'envira_gallery_updates_' . $this->plugin_slug, $timestamp );
        } else {
            ...
            if ( $current_timestamp < $timestamp && 'update-core.php' !== $pagenow && 'update.php' !== $pagenow && ! ( isset( $_GET['tab'] ) && 'plugin-information' == $_GET['tab'] && isset( $_GET['plugin'] ) && $this->plugin_slug == $_GET['plugin'] ) ) {
                ...
                update_option( 'envira_gallery_updates_' . $this->plugin_slug, strtotime( '+12 hours' ) );
            }
        }
