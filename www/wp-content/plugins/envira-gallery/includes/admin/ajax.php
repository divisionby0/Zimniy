...
add_action( 'wp_ajax_envira_gallery_library_search', 'envira_gallery_ajax_library_search' );
/**
 * Searches the Media Library for images matching the term specified in the search.
 *
 * @since 1.0.0
 */
function envira_gallery_ajax_library_search() {
...
function envira_gallery_ajax_save_meta() {
    ...
    if ( isset( $meta['title'] ) ) {
        $gallery_data['gallery'][$attach_id]['title'] = trim( esc_html( $meta['title'] ) );
    }
